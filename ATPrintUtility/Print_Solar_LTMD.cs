...
namespace AT.Print
{
    public partial class Print_Solar_LTMD : UserControl
    {
        public Print_Solar_LTMD()
        {
            InitializeComponent();
            BindPrinters();
        }
        ...
        SolarBill parseSolarBill(DataTable dtSolarBill)
        {
            ...
            sht.L1_FeederName = dtSolarBill.Rows[0][9].ToString();
            sht.L1_TODOrNon_TODFlag = dtSolarBill.Rows[0][10].ToString();
            sht.L1_AKY_indicator = dtSolarBill.Rows[0][11].ToString();
            sht.L1_DisconnectionMSGPrintingIMMEDIATE = dtSolarBill.Rows[0][12].ToString();
            sht.L1_BillingCode = dtSolarBill.Rows[0][13].ToString();
            if (dtSolarBill.Rows[0][14].ToString() == "")
            {
                sht.L1_Customer_PAN = dtSolarBill.Rows[0][14].ToString();
            }
            else
            {
                sht.L1_Customer_PAN = "PAN No: " + dtSolarBill.Rows[0][14].ToString();
            }
            sht.L1_MobileNumber = "Registered Mobile : " + dtSolarBill.Rows[0][15].ToString();
            sht.L1_EmailId = "Registered Email Id : " + dtSolarBill.Rows[0][16].ToString();
            sht.L1_Net = dtSolarBill.Rows[0][17].ToString(); //for net metering/billing
            sht.L1_BillDays = dtSolarBill.Rows[0][18].ToString();
            ...
            sht.L8_amount_payable_before_due_date = dtSolarBill.Rows[7][11].ToString().Trim('�');
            ...
            sht.L22_TOD_2_KWH = dtSolarBill.Rows[21][1].ToString();
            sht.L22_TOD_3_KWH = dtSolarBill.Rows[21][2].ToString();
            sht.L22_TOD_4_KWH = dtSolarBill.Rows[21][3].ToString();
            //   Line 22 End
            #endregion
            ...
            sht.L23_TOD_2_KW = dtSolarBill.Rows[22][1].ToString();
            sht.L23_TOD_3_KW = dtSolarBill.Rows[22][2].ToString();
            sht.L23_TOD_4_KW = dtSolarBill.Rows[22][3].ToString();
            ...
            sht.L24_TOD_2_KWH = dtSolarBill.Rows[23][1].ToString();
            sht.L24_TOD_3_KWH = dtSolarBill.Rows[23][2].ToString();
            sht.L24_TOD_4_KWH = dtSolarBill.Rows[23][3].ToString();
            ...
            sht.L25_TOD_2_KWH = dtSolarBill.Rows[24][1].ToString();
            sht.L25_TOD_3_KWH = dtSolarBill.Rows[24][2].ToString();
            sht.L25_TOD_4_KWH = dtSolarBill.Rows[24][3].ToString();
        ...
        private void sbLoadServiceNoBasedMessages_Click(object sender, EventArgs e)
        {
            ...
            using (OpenFileDialog ofdMv = new OpenFileDialog())
            {
                ...
                if (ofdMv.ShowDialog() == DialogResult.OK)
                {
                    ...
                    AppFunctions.ShowWaitForm("Generating Solar LTMD Bills Now..!!");
        ...
        private bool ValidatetxtFile(string[] Bills)
        {
            try
            {
                ...
                {
                    BillNo++;
                    DataTable dtSolarBill = ParseAsDataTable.LTMD_Solar_FileTxtToDataTable(Bill, BillNo, "LTMD Solar");
