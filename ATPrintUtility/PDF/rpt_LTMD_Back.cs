...
{
    public partial class rpt_LTMD_Back : DevExpress.XtraReports.UI.XtraReport
    {
        public rpt_LTMD_Back()
        {
            InitializeComponent();
        }
        ...
        {
            var op = this.DataSource as List<SingleLTMDBill>;
                ...
                if (op[0].L1_BillingCode == "3000")
                {
                    if (!IsMessageLimitExceeds(totalMessages))
                    {
                        ...
                        XRLabel xrMessageTheftAmount = new XRLabel
                        {
                            Font = new DXFont("Noto Sans Devanagari", 8),
                            TextAlignment = DevExpress.XtraPrinting.TextAlignment.MiddleLeft,
                            Text = string.Format(getMessage(LoadStaticData._HindiMessage, "AB1"), totalUnits.ToString(), PrevReadDt, ReadDt, op[0].L10_Mode + " दिन"),
                            WordWrap = true,
                            AutoWidth = true,
                            KeepTogether = true,
                            HeightF = 0.1f,
